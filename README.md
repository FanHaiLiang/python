...
# if语句
#### if 判断条件: 语句块/pass　　　pass:占位符　什么都不执行
#### if 判断条件：语句块　else:(如果if条件不成立时执行这个)　语句块
#### if 判断条件１:语句块 elif 判断条件２:(当判断条件１不成立时执行这个 可以嵌套无数个这个) 语句块 else:(当判断条件１/２都不成立时　执行这个)语句块
## 三元运算：
